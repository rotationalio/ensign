...
func (suite *tenantTestSuite) TestTenantProjectPatch() {
	...
	testCases := []struct {
		...
		{&api.Project{Description: strings.Repeat("a", db.MaxDescriptionLength+1)}, (&db.ValidationError{Field: "description", Err: db.ErrProjectDescriptionTooLong, Index: -1}).Error()},
